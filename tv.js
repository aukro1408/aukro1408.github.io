...
//Стиль
Lampa.Template.add(plugin.component + '_style', '<style>#PLUGIN{width:100%;padding:0;margin:0;box-sizing:border-box}.PLUGIN.category-full{width:100%;padding:0 0 8em;margin:0;box-sizing:border-box}.PLUGIN .card--collection{width:100%!important;float:none!important;margin:0!important;padding:0!important;box-sizing:border-box}.PLUGIN .card--collection .card__view{position:relative!important;width:100%!important;height:5.7em!important;padding:0!important;margin:0 0 .35em!important;border-radius:.45em!important;background:rgba(255,255,255,.045)!important;border:1px solid rgba(255,255,255,.045)!important;overflow:hidden!important;display:flex!important;align-items:stretch!important;box-sizing:border-box!important}.PLUGIN .card--collection.focus .card__view{background:rgba(255,255,255,.11)!important;border-color:rgba(244,180,0,.7)!important;box-shadow:inset 3px 0 0 #f4b400!important}.PLUGIN .card__img{position:absolute!important;left:.7em!important;top:50%!important;transform:translateY(-50%)!important;width:5.2em!important;height:3.7em!important;object-fit:contain!important;background:transparent!important;border-radius:.25em!important;font-size:1.05em!important;display:block!important}.PLUGIN .card__title,.PLUGIN .card__meta,.PLUGIN .card__age,.PLUGIN .card__icons{display:none!important}.PLUGIN .max_iptv_channel{position:relative;width:22%;min-width:11em;height:100%;flex-shrink:0;box-sizing:border-box;padding:0 .65em 0 6.2em;display:flex;flex-direction:column;justify-content:center;overflow:hidden;border-right:1px solid rgba(255,255,255,.08)}.PLUGIN .max_iptv_channel_name{font-size:1.05em;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#fff;line-height:1.25}.PLUGIN .max_iptv_channel_num{font-size:.72em;opacity:.42;margin-top:.18em;white-space:nowrap}.PLUGIN .max_iptv_programs{height:100%;min-width:0;flex:1;display:flex;align-items:stretch;overflow:hidden}.PLUGIN .max_iptv_program{position:relative;min-width:7.2em;height:100%;padding:.65em .8em .45em;box-sizing:border-box;border-right:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.018);overflow:hidden;display:flex;flex-direction:column;justify-content:center}.PLUGIN .max_iptv_program.now{background:rgba(255,255,255,.075);box-shadow:inset 0 -3px 0 rgba(244,180,0,.9)}.PLUGIN .max_iptv_program_title{font-size:.92em;color:#f2f2f2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.25}.PLUGIN .max_iptv_program_time{font-size:.68em;opacity:.45;margin-top:.28em;white-space:nowrap}.PLUGIN .max_iptv_program_progress{position:absolute;left:0;bottom:0;height:3px;background:#f4b400;width:0}.PLUGIN .max_iptv_empty{height:100%;display:flex;align-items:center;padding:0 1em;opacity:.35;font-size:.85em}.PLUGIN .max_iptv_guide_head{display:flex;height:2.7em;margin:0 0 .35em;padding:0;box-sizing:border-box;align-items:center;opacity:.75}.PLUGIN .max_iptv_guide_head_channel{width:22%;min-width:11em;padding-left:1em;box-sizing:border-box;font-size:.78em;letter-spacing:.04em;text-transform:uppercase;opacity:.55}.PLUGIN .max_iptv_guide_head_time{flex:1;height:100%;display:flex;align-items:center;justify-content:space-between;padding:0 .9em;box-sizing:border-box;font-size:.76em;border-left:1px solid rgba(255,255,255,.08)}.PLUGIN .max_iptv_guide_head_now{color:#f4b400;font-weight:600}.PLUGIN .max_iptv_info{padding:.7em 1em .8em!important;margin:0!important}.PLUGIN .max_iptv_info .info__title{font-size:1.35em!important}.PLUGIN .max_iptv_info .info__title-original{opacity:.45;font-size:.8em!important}.PLUGIN .max_iptv_info .info__create{opacity:.65;font-size:.82em!important}@media(max-width:800px){.PLUGIN .max_iptv_channel,.PLUGIN .max_iptv_guide_head_channel{width:30%;min-width:9em}.PLUGIN .max_iptv_channel{padding-left:5em}.PLUGIN .card__img{width:4.1em!important;height:3.1em!important;left:.45em!important}.PLUGIN .max_iptv_program{min-width:6.5em;padding-left:.55em;padding-right:.55em}}@media(max-width:500px){.PLUGIN .card--collection .card__view{height:5.1em!important}.PLUGIN .max_iptv_channel,.PLUGIN .max_iptv_guide_head_channel{width:34%;min-width:7.2em}.PLUGIN .max_iptv_channel{padding-left:4.25em}.PLUGIN .card__img{width:3.45em!important;height:2.6em!important}.PLUGIN .max_iptv_channel_name{font-size:.88em}.PLUGIN .max_iptv_program_title{font-size:.78em}.PLUGIN .max_iptv_program_time{font-size:.61em}}</style>'.replace(/PLUGIN/g, plugin.component));
...
function pluginPage(object) {
    ...
    guideTimer = setInterval(function(){
        for (var gid in guideRows) renderGuideRow(gid);
    }, 1000);
    ...
    function guideTime(minute){ return toLocaleTimeString(minute * 60000); }
    function renderGuideRow(epgId){
        var row = guideRows[epgId];
        if (!row) return;
        var epg = (EPG[epgId] && EPG[epgId][2]) || [];
        if (epg === false) return;
        var now = Math.floor(unixtime()/60);
        while (epg.length && now >= epg[0][0] + epg[0][1]) epg.shift();
        var box = row.find('.max_iptv_programs');
        if (!box.length) return;
        box.empty();
        if (!epg.length) { box.append('<div class="max_iptv_empty">Телепрограмма недоступна</div>'); return; }
        var maxItems = Math.min(epg.length, 5);
        for (var i=0;i<maxItems;i++){
            var e=epg[i];
            var isNow = now >= e[0] && now < e[0]+e[1];
            var mins = Math.max(1, e[1]);
            var item = $('<div class="max_iptv_program'+(isNow?' now':'')+'"></div>');
            item.css('flex',''+Math.max(1,mins/30)+' 1 0');
            item.append('<div class="max_iptv_program_title"></div>');
            item.append('<div class="max_iptv_program_time"></div>');
            item.find('.max_iptv_program_title').text(e[2] || 'Без названия');
            item.find('.max_iptv_program_time').text(guideTime(e[0])+' — '+guideTime(e[0]+e[1]));
            if (isNow){
                var progress=Math.max(0,Math.min(100,Math.round((unixtime()-e[0]*60)*100/(e[1]*60||60))));
                item.append('<div class="max_iptv_program_progress"></div>');
                item.find('.max_iptv_program_progress').css('width',progress+'%');
            }
            box.append(item);
        }
    }
    function prepareGuideRow(card, channel){
        var view=card.find('.card__view');
        var img=card.find('.card__img');
        view.addClass('max_iptv_guide_view');
        view.find('.max_iptv_channel,.max_iptv_programs').remove();
        var channelCell=$('<div class="max_iptv_channel"></div>');
        channelCell.append(img);
        channelCell.append('<div class="max_iptv_channel_name"></div>');
        channelCell.append('<div class="max_iptv_channel_num"></div>');
        channelCell.find('.max_iptv_channel_name').text(channel.Title);
        channelCell.find('.max_iptv_channel_num').text('Канал '+channel.ChNum);
        var programs=$('<div class="max_iptv_programs"><div class="max_iptv_empty">Загрузка EPG…</div></div>');
        view.prepend(channelCell).append(programs);
        if (channel['tvg-logo']) img.attr('src',channel['tvg-logo']);
        else {
            img.hide();
        }
        return {channel:channel, row:card, programs:programs};
    }
	...
	var bulkFn = bulkWrapper(function (channel) {
		...
		Lampa.Select.show({
			...
			if (!!sel.archive) {
			    ...
			    setStorage('epg', !getStorage('epg', false));
			    Lampa.Controller.toggle('content');
			} else {
	...
	if (data.length) {
	    ...
	    var guideHead = $('<div class="max_iptv_guide_head"><div class="max_iptv_guide_head_channel">Канал</div><div class="max_iptv_guide_head_time"><span class="max_iptv_guide_head_now">СЕЙЧАС</span><span>ДАЛЕЕ</span><span>ПОЗЖЕ</span></div></div>');
	    html.append(guideHead);
