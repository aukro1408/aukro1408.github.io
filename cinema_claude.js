...
(function(){
    ...
    // =================================================================
    // TMDB REQUEST CACHE
    // Одні й ті самі категорії/деталі фільмів запитувались заново при
    // кожному відкритті рядка чи картки. TTL-кеш в пам'яті знижує
    // кількість запитів до TMDB і прискорює повторні відкриття.
    // =================================================================
    var TMDB_CACHE = {};
    var TMDB_CACHE_TTL = 5 * 60 * 1000; // 5 хвилин
    var TMDB_CACHE_MAX_ENTRIES = 200;

    function cachedTmdbRequest(network, url, onSuccess, onError) {
        var cached = TMDB_CACHE[url];
        if (cached && (Date.now() - cached.time) < TMDB_CACHE_TTL) {
            onSuccess(cached.data);
            return;
        }
        network.silent(url, function (json) {
            var keys = Object.keys(TMDB_CACHE);
            if (keys.length >= TMDB_CACHE_MAX_ENTRIES) {
                // Найпростіша евікція: прибираємо найстаріший запис, щоб кеш не ріс необмежено.
                keys.sort(function (a, b) { return TMDB_CACHE[a].time - TMDB_CACHE[b].time; });
                delete TMDB_CACHE[keys[0]];
            }
            TMDB_CACHE[url] = { data: json, time: Date.now() };
            onSuccess(json);
        }, onError);
    }
    ...
    // Единственное місце пошуку та запуску трейлера фільму/серіалу.
    // Раніше ця логіка була продубльована 3 рази (кнопка на hero-банері,
    // меню onlyEnter, обробник кнопки Play onKey) — тепер лише один виклик.
    function findAndPlayTrailer(movie) {
        ...
        function search(searchLang) {
            ...
            cachedTmdbRequest(network, url, function (json) {
    ...
    function makeHeroResultItem(movie, heightEm) {
        ...
        var renderHeroContent = function(item, movie) {
            ...
            var network = new Lampa.Reguest();
            cachedTmdbRequest(network, url, function(details) {
                // Logo
                var logo = null;
                ...
                return function (callback) {
                    ...
                    cachedTmdbRequest(network, url, function (json) {
                        ...
                        if (!items.length) {
                            ...
                            cachedTmdbRequest(network, url, function (retryJson) {
