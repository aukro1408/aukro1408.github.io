(function () {
    ...
    // Сортировка по умолчанию при открытии рецензий
    const DEFAULT_ORDER_STORAGE = "kp_reviews_default_order";

    // Показывать ли отзывы, похожие на спойлерные, сразу
    const SHOW_SPOILERS_STORAGE = "kp_reviews_show_spoilers";
    ...
    // Простое склонение слова "фильм" под число
    function pluralizeFilms(count) {
    ...
    // Грубая эвристика: в API нет официального флага спойлера,
    // поэтому ищем характерное слово в заголовке/тексте отзыва
    function isSpoilerReview(review) {
        const combined = (
            (review.title || "") +
            " " +
        ...
        return combined.indexOf("спойлер") !== -1;
    }
    ...
    function renderReviewTextBlock(
        review,
        description,
        index
    ) {
            ...
            <div class="kp-review-text-wrap">
                ...
                ${
                    isSpoiler
                        ? `
                            <div
                                class="kp-review-spoiler"
                                id="kp-review-spoiler-${index}"
                            >
                                <span class="kp-review-spoiler-icon">
                                    ⚠️
                                </span>

                                <span class="kp-review-spoiler-text">
                                    Похоже, отзыв содержит спойлеры
                                </span>

                                <div
                                    class="kp-review-spoiler-btn selector"
                                    data-index="${index}"
                                >
                                    Показать текст
                                </div>
    ...
    function addStyles() {
        ...
        style.textContent = `
            ...
            .kp-review-spoiler-icon {
                font-size: 15px;
            }

            .kp-review-spoiler-text {
                color: #e0c8a0;
                font-size: 13px;
                flex: 1;
                min-width: 140px;
            ...
            .kp-review-spoiler-btn {
                ...
                font-weight: 700;
                cursor: pointer;
                box-shadow: 0 6px 14px rgba(255,152,0,.3);
                transition: transform .15s ease, box-shadow .15s ease;
            }

            .kp-review-spoiler-btn.focus,
            .kp-review-spoiler-btn:hover {
                transform: translateY(-1px);
                box-shadow: 0 10px 18px rgba(255,152,0,.4);
            }
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                name: DEFAULT_ORDER_STORAGE,
                type: "select",

                values: ORDER_OPTIONS.reduce(
                    function (acc, o) {
                        acc[o.value] = o.label;
                        return acc;
                    },
            ...
            field: {
                ...
                description:
                    "В каком порядке показывать отзывы при открытии."
            }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                name: SHOW_SPOILERS_STORAGE,
                type: "trigger",
                "default": false
            },

            field: {
                name: "Показывать спойлеры",
                description:
                    "Если выключено — отзывы со словом «спойлер» в тексте скрываются за кнопкой «Показать текст». API не даёт официальной метки спойлера, определяем по тексту."
            }
        });


        // -----------------------------------------------------
        // Размер кэша соответствий
        // -----------------------------------------------------
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: "Размер кэша",
                description:
                    "Нажмите, чтобы узнать, сколько фильмов сейчас сохранено в локальном кэше соответствий."
            },
