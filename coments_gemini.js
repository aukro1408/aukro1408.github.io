(function () {
    ...
    function formatDuration(minutes) {
        ...
        if (mins > 0 || hours === 0) res += mins + " мин";
        return res.trim();
    }
    ...
    function formatMoney(amount, currency = "$") {
        if (!amount) return "";
        return Number(amount).toLocaleString("ru-RU") + " " + (currency === "USD" ? "$" : currency);
    }
    ...
    function renderRatings(film) {
        if (!film) return "";
        const kpRating = film.ratingKinopoisk ? film.ratingKinopoisk.toFixed(1) : null;
        const kpVotes = film.ratingKinopoiskVoteCount ? film.ratingKinopoiskVoteCount.toLocaleString("ru-RU") : "0";
        
        const imdbRating = film.ratingImdb ? film.ratingImdb.toFixed(1) : null;
        const imdbVotes = film.ratingImdbVoteCount ? film.ratingImdbVoteCount.toLocaleString("ru-RU") : "0";

        if (!kpRating && !imdbRating) return "";

        return `
            <div class="kp-ratings-block">
                ${kpRating ? `
                    <div class="kp-rating-card kp-rating-kp">
                        <div class="kp-rating-source">Кинопоиск</div>
                        <div class="kp-rating-value">${kpRating}</div>
                        <div class="kp-rating-votes">${kpVotes} голосов</div>
                    </div>
                ` : ""}
                ${imdbRating ? `
                    <div class="kp-rating-card kp-rating-imdb">
                        <div class="kp-rating-source">IMDb</div>
                        <div class="kp-rating-value">${imdbRating}</div>
                        <div class="kp-rating-votes">${imdbVotes} голосов</div>
                    </div>
                ` : ""}
            </div>
        `;
    ...
    function renderMainInfo(film) {
        if (!film) return "";
        const slogan = film.slogan ? `«${film.slogan}»` : "";
        const ageLimit = formatAgeLimit(film.ratingAgeLimits || film.ageLimits);
        const duration = formatDuration(film.filmLength);

        if (!slogan && !ageLimit && !duration) return "";

        return `
            <div class="kp-info-section kp-main-info">
                ${slogan ? `<div class="kp-slogan">${escapeHtml(slogan)}</div>` : ""}
                <div class="kp-info-badges">
                    ${ageLimit ? `<span class="kp-badge kp-age">${escapeHtml(ageLimit)}</span>` : ""}
                    ${duration ? `<span class="kp-badge kp-duration">${escapeHtml(duration)}</span>` : ""}
                </div>
            </div>
        `;
    }

    function renderFinances(boxOfficeItems) {
        if (!boxOfficeItems || !boxOfficeItems.length) return "";
        ...
        boxOfficeItems.forEach(item => {
            ...
            if (type === "BUDGET") budget = formatMoney(item.amount, item.currency);
            if (type === "US") usBox = formatMoney(item.amount, item.currency);
            if (type === "WORLD") worldBox = formatMoney(item.amount, item.currency);
            if (type === "RUSSIA") russiaBox = formatMoney(item.amount, item.currency);
        });

        if (!budget && !usBox && !worldBox && !russiaBox) return "";

        return `
            <div class="kp-info-section">
                <div class="kp-info-title">💰 Финансы</div>
                <div class="kp-finances-grid">
                    ${budget ? `<div class="kp-finance-item"><span>Бюджет:</span> <strong>${budget}</strong></div>` : ""}
                    ${usBox ? `<div class="kp-finance-item"><span>Сборы в США:</span> <strong>${usBox}</strong></div>` : ""}
                    ${worldBox ? `<div class="kp-finance-item"><span>Сборы в мире:</span> <strong>${worldBox}</strong></div>` : ""}
                    ${russiaBox ? `<div class="kp-finance-item"><span>Сборы в России:</span> <strong>${russiaBox}</strong></div>` : ""}
                </div>
            </div>
        `;
    }
    ...
    function renderAwards(awards) {
        ...
        const displayed = awards.slice(0, 3);
        const remaining = total - 3;

        let html = `<div class="kp-info-section">
            <div class="kp-info-title">🏆 Награды и номинации</div>
            <div class="kp-awards-list">`;
        
        displayed.forEach(award => {
            const name = award.name || award.nominationName || "Награда";
            html += `<div class="kp-award-item">✦ ${escapeHtml(name)}</div>`;
        });

        if (remaining > 0) {
            html += `<div class="kp-award-more">...и ещё ${remaining}</div>`;
        }
    ...
    function renderSequels(items) {
        ...
        let html = `<div class="kp-info-section">
            <div class="kp-info-title">📚 Связанные фильмы</div>
            <div class="kp-sequels-list">`;

        items.forEach(item => {
            const title = item.nameRu || item.nameEn || item.nameOriginal || "Фильм";
            const year = item.year ? ` (${item.year})` : "";
            const relType = typeLabels[item.relationType] || item.relationType || "";
            html += `<div class="kp-sequel-item">
                <span class="kp-sequel-type">${escapeHtml(relType)}:</span>
                <span class="kp-sequel-title">${escapeHtml(title)}${year}</span>
            </div>`;
        });
    ...
    function addStyles() {
        ...
        style.textContent = `
            ...
            .kp-rating-kp .kp-rating-value {
                color: #ff9800;
            }

            .kp-rating-imdb .kp-rating-value {
                color: #f5c518;
            }
    ...
    function setupSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name:
                    "Модуль показывает рейтинги, информацию и рецензии Кинопоиска на странице фильма/сериала. Для работы нужен бесплатный API-ключ."
            }
        });
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                ...
                description:
                    "Если для фильма подтянулись не те данные — сотрите локальный кэш соответствий TMDB/Kinopoisk ID."
            },
    ...
    function createModal() {
        ...
        const html = `
            <div class="kp-reviews-container">
                ...
                >
                    Данные предоставлены Kinopoisk API
                </div>
    ...
    async function loadReviewPage(
        ...
        try {
            ...
            // Рендерим новые информационные блоки наверх
            let html = "";
            html += renderRatings(currentFilmData);
            html += renderMainInfo(currentFilmData);
            html += renderFinances(currentBoxOffice);
            html += renderAwards(currentAwards);
            html += renderSequels(currentSequels);
    ...
    // =========================================================
    // ОТКРЫТИЕ МОДАЛКИ С ДАННЫМИ И ОТЗЫВАМИ
    // =========================================================
    ...
    async function openReviews(
        ...
        try {
            ...
            // Параллельно загружаем основную информацию, финансы, награды и сиквелы
            const [filmData, boxOffice, awards, sequels] = await Promise.all([
                kpFetch(`/api/v2.2/films/${currentKinopoiskId}`).catch(() => null),
                kpFetch(`/api/v2.2/films/${currentKinopoiskId}/box_office`).catch(() => ({ items: [] })),
                kpFetch(`/api/v2.2/films/${currentKinopoiskId}/awards`).catch(() => ({ items: [] })),
                kpFetch(`/api/v2.1/films/${currentKinopoiskId}/sequels_and_prequels`).catch(() => [])
            ]);


            currentFilmData = filmData;
            currentBoxOffice = boxOffice.items || boxOffice || [];
            currentAwards = awards.items || awards || [];
            currentSequels = Array.isArray(sequels) ? sequels : (sequels.items || []);
