...
(function(){
    ...
    function ensureCinemaXStreamingCleanup() {
        if (window.__cinemaxStreamingCleanupReady) return;
        window.__cinemaxStreamingCleanupReady = true;
        setInterval(function () {
            try {
                if (!$('.category-full').length && $('body').hasClass('cinemax-streaming-layout')) {
                    $('body').removeClass('cinemax-streaming-layout');
                }
            } catch (e) {}
        }, 800);
    }
    ...
    function StudiosMain(object) {
        ...
        comp.onMore = function (data) {
            // IMPORTANT: use the same native Lampa category_full component
            // that the working Horror page uses. The previous custom
            // StudiosView passed display params instead of TMDB query params,
            // which also prevented the Horror-style grid from being reused.
            try { $('body').addClass('cinemax-streaming-layout'); } catch (e) {}
