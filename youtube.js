// ==UserScript==
// @name         Lampa YouTube Button (Stable Loader)
// @namespace    lampa.youtube.stable
// @version      1.1
// @description  Гарантированное появление кнопки YouTube в меню Lampa
// ==/UserScript==
...
(function () {
  ...
  function addButton() {
    try {
      if (window.Lampa && Lampa.Plugin && typeof Lampa.Plugin.create === 'function') {
        console.log('✅ Lampa найдена, создаю кнопку YouTube');
        ...
        Lampa.Noty.show('Плагин YouTube успешно загружен 🎬');
        return true;
      }
    } catch (e) {
  ...
  function waitForLampa() {
    const ok = addButton();
    if (!ok) {
      console.log('⏳ Жду загрузку Lampa...');
