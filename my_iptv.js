...
function pluginPage(object) {
    ...
    body.toggleClass('full_width_channels', getSettings('full_width_channels'));
    var info;
    var last;
...
langAdd('full_width_channels', {
    ru: 'Список каналов во всю ширину',
    uk: 'Список каналів на всю ширину',
    be: 'Спіс каналаў ва ўсю шырыню',
    en: 'Full-width channel list',
    zh: '全宽频道列表'
});
langAdd('full_width_channels_desc', {
    ru: 'Логотип, название канала и текущая передача в одной строке вместо квадратной сетки',
    uk: 'Логотип, назва каналу і поточна передача в одному рядку замість квадратної сітки',
    be: 'Лагатып, назва канала і бягучая перадача ў адным радку замест квадратнай сеткі',
    en: 'Logo, channel name and current programme in one row instead of a square grid',
    zh: '徽标、频道名称和当前节目显示在一行中,而不是方形网格'
});
...
addSettings(
    'trigger',
    {
	title: langGet('full_width_channels'),
	description: langGet('full_width_channels_desc'),
	name: 'full_width_channels',
	default: true,
	onChange: function(v){
	    $('.' + plugin.component + '.category-full').toggleClass('full_width_channels', v === 'true');
	}
    }
);
