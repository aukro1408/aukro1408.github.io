...
// --- ИЗМЕНЕННЫЙ БЛОК СТИЛЕЙ ДЛЯ СПИСКА ---
...
'position:relative;' +
'background-color:#2a2a2a;' + /* Чуть темнее фон для списка */
'border-radius:0.5em;' +
'cursor:pointer;' +
'padding-bottom:0 !important;' + /* Убираем отступ для квадрата */
'height: 4.5em;' + /* Фиксированная высота строки */
'display: flex;' +
'align-items: center;' +
'overflow: hidden;' +
'margin-bottom: 0.5em;' +
...
'border-radius:0.3em;' +
'max-height:80%;' +
'max-width:20%;' + /* Ограничиваем ширину лого */
'width: 4em;' +
'height: 3em;' +
'object-fit: contain;' +
...
/* Блок с программой передач (всегда видим в списке) */
'.PLUGIN .card__age{' +
'padding:0;' +
'border:none;' +
'margin-top:0;' +
...
/* Прогресс бар программы */
'.PLUGIN .card__age .card__epg-progress{' +
'position:absolute;' +
'background-color:#f1c40f;' + /* Желтый цвет как на скрине */
'bottom: -0.4em;' + /* Линия внизу карточки */
'left:0;' +
'width:0%;' +
'max-width:100%;' +
'height:0.2em;' +
...
/* Текст программы */
'.PLUGIN .card__age .card__epg-title{' +
'position:relative;' +
'padding:0;' +
'text-overflow:ellipsis;' +
'white-space:nowrap;' +
'overflow:hidden;' +
'font-size: 0.9em;' +
'color: #f1c40f;' + /* Желтоватый текст программы */
'opacity: 0.8;' +
'}' +

/* Если программы нет, скрываем блок, чтобы не было пустоты (опционально) */
'.PLUGIN .card__age:empty { display: none !important; }' +
...
/* Убираем сетку, делаем список */
'.PLUGIN .card--collection { width: 100% !important; margin: 0 0 0.5em 0 !important; float: none !important; }' +
...
// Убираем класс square_icons, так как нам нужен список
...
;
 ...
 var bulkFn = bulkWrapper(function (channel) {
     ...
     // --- ДОБАВЛЯЕМ ОБЕРТКУ ДЛЯ ТЕКСТА, ЧТОБЫ ВЫРОВНЯТЬ ЕГО РЯДОМ С ЛОГО ---
     // Находим view (основной контейнер карточки)
     var view = card.find('.card__view');
     // Создаем контейнер для контента
     var contentDiv = $('<div class="card__content"></div>');
     // Перемещаем заголовок внутрь нового контейнера
     var titleEl = card.find('.card__title');
     titleEl.appendTo(contentDiv);
     // Перемещаем блок возраста/EPG внутрь нового контейнера
     var ageEl = card.find('.card__age');
     ageEl.appendTo(contentDiv);
     // Добавляем контейнер в view
     view.append(contentDiv);
     // -------------------------------------------------------------
     ...
     // Инициализация блока EPG внутри карточки
     card.find('.card__age').html('<div class="card__epg-progress js-epgProgress"></div><div class="card__epg-title js-epgTitle">Загрузка...</div>');
 ...
 // Убираем стили сетки из шаблона кнопки категории, чтобы не мешали
...
// Игнорируем настройку квадратных иконок, так как теперь всегда список
}
}
);
addSettings(
