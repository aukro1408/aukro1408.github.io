(function () {
    ...
    function setupSettings() {
        ...
        // Важно: SettingsApi сам добавляет имя компонента к param.name.
        if (!Lampa.SettingsApi ||
            typeof Lampa.SettingsApi.addComponent !== 'function' ||
            typeof Lampa.SettingsApi.addParam !== 'function') {
            console.warn('[IPTV] SettingsApi недоступен');
            return;
        }

        // Защита от повторной регистрации.
        if (window.__simple_iptv_settings_registered) return;
        ...
        try {
            ...
            function addSettings(type, param) {
                ...
                var data = {
                    ...
                    param: {
                        name: PLUGIN.component + '_' + (param.name || 'setting'),
                        type: type,
                        values: typeof param.values === 'undefined' ? '' : param.values,
                        placeholder: typeof param.placeholder === 'undefined' ? '' : param.placeholder,
                ...
                if (param.description) {
                    data.field.description = param.description;
                }

                if (typeof param.onChange === 'function') {
                    data.onChange = param.onChange;
                }

                if (typeof param.onRender === 'function') {
                    data.onRender = param.onRender;
                }
            ...
            addSettings('input', {
                name: 'url',
                title: 'URL плейлиста M3U',
                placeholder: DEFAULT_PLAYLIST,
                default: getPlaylistUrl() || DEFAULT_PLAYLIST,
                description: 'Ссылка на ваш M3U или M3U8 плейлист с каналами',
            ...
            addSettings('input', {
                name: 'epg',
                title: 'URL программы EPG',
                placeholder: 'https://example.com/epg.xml',
                default: getEpgUrl(),
                description: 'XMLTV. Если поле пустое, используется url-tvg из M3U',
                onChange: function (value) {
                    setEpgUrl(value);
            ...
            window.__simple_iptv_settings_registered = true;
            console.log('[IPTV] Настройки зарегистрированы');
        } catch (e) {
            console.error('[IPTV] Ошибка регистрации настроек:', e);
            // Не ставим флаг — можно повторить регистрацию после готовности Lampa.
        }
    }
