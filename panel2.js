// realistic_snow_control_lampa_ui.js
(function(){
  if(window.__lampa_realistic_snow_ui_inited) return;
  window.__lampa_realistic_snow_ui_inited = true;
  ...
  function initSnow(){
    ...
    // --- Панель управления с красивым UI ---
    var ctl = document.createElement('div');
    Object.assign(ctl.style,{
      position:'fixed', right:'15px', bottom:'15px', zIndex:1000000,
      background:'rgba(20,20,20,0.8)', color:'#fff', padding:'10px',
      borderRadius:'8px', fontFamily:'Arial', fontSize:'12px',
      pointerEvents:'auto', backdropFilter:'blur(6px)',
      boxShadow:'0 4px 12px rgba(0,0,0,0.5)', minWidth:'180px'
    });
    ctl.innerHTML=`
      ...
      <label style="display:flex;align-items:center;gap:6px">
        <input type="checkbox" id="snow_enabled"> Enabled
      </label>
      <label style="display:flex;align-items:center;gap:6px;margin-top:6px">
        Density <input type="range" min="20" max="300" id="snow_density" style="flex:1;transition:0.2s">
      </label>
      <label style="display:flex;align-items:center;gap:6px;margin-top:6px">
        Speed <input type="range" min="0.2" max="3" step="0.1" id="snow_speed" style="flex:1;transition:0.2s">
      </label>
      <label style="display:flex;align-items:center;gap:6px;margin-top:6px">
        Min size <input type="range" min="0.5" max="3" step="0.1" id="snow_minSize" style="flex:1;transition:0.2s">
      </label>
      <label style="display:flex;align-items:center;gap:6px;margin-top:6px">
        Max size <input type="range" min="1" max="6" step="0.1" id="snow_maxSize" style="flex:1;transition:0.2s">
      </label>
    `;
    ...
    console.info('Lampa Snow plugin with beautiful UI initialized.');
  }
